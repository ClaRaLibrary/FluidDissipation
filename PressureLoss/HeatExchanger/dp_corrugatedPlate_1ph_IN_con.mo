...
record dp_corrugatedPlate_1ph_IN_con "Input record for function dp_corrugatedPlate_1ph, dp_corrugatedPlate_1ph_DP"
  extends Modelica.Icons.Record;

  Integer channels(min = 1) = 0 "number of paralell flown channels per fluid" annotation (Dialog(group="HeatExchanger"));
  SI.Length Length(min=1e-2)=0 "length of the heat exchanger plates in flow direction (header center to header center)"
    annotation (Dialog(group="HeatExchanger"));
  SI.Length Width(min=1e-2)=0 "width of the heat exchanger plates in flow direction"
                                                           annotation (Dialog(group="HeatExchanger"));
