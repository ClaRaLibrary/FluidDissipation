...
function dp_edgedOverall_MFLOW "Pressure loss of edged bend | calculate mass flow rate | overall flow regime | surface roughness"
  ...
  //input records
  input FluidDissipation.PressureLoss.Bend.dp_edgedOverall_IN_con IN_con "Input record for function dp_edgedOverall_MFLOW"
    annotation (Dialog(group="Constant inputs"));
  input FluidDissipation.PressureLoss.Bend.dp_edgedOverall_IN_var IN_var "Input record for function dp_edgedOverall_MFLOW"
...
protected
  ...
  SI.ReynoldsNumber Re_lam_min=500 "Start of transition regime for roughness contribution";
...
//   Real k_Re = B/(max(MIN, velocity)*IN_con.d_hyd*IN_var.rho)^exp*IN_var.eta^exp;
  ...
  SI.Velocity v_min = Re_min*IN_var.eta/(IN_var.rho*d_hyd) "Minimum mean velocity for regularization";

  SI.Pressure dp_min=A*C1*zeta_LOC*(B/2)*(d_hyd/IN_var.eta)^(-exp)*IN_var.rho^(1 - exp)
      *v_min^(pow) "Linear smoothing of mass flow rate for decreasing pressure loss";

  SI.Velocity v_lam_min = Re_lam_min*IN_var.eta/(IN_var.rho*d_hyd) "Mean velocity for starting of transition to roughness regime";
  SI.Velocity v_lam_leave = Re_lam_leave*IN_var.eta/(IN_var.rho*d_hyd) "Mean velocity for end of transition to roughness regime";

  SI.Pressure dp_lam_min=A*C1*zeta_LOC*(B/2)*(d_hyd/IN_var.eta)^(-exp)*IN_var.rho^(1 - exp)
      *v_lam_min^(pow) "Pressure loss for starting of transition to roughness regime";

  TYP.DarcyFrictionFactor lambda_lam_leave_rough=0.25/(Modelica.Math.log10(k/(3.7*
      IN_con.d_hyd) + 5.74/Re_lam_leave^0.9))^2 "Darcy friction factor considering surface roughness at Re_lem_leave";
  TYP.DarcyFrictionFactor lambda_lam_leave_smooth=0.25/(Modelica.Math.log10(5.74/Re_lam_leave^0.9))^2 "Darcy friction factor neglecting surface roughness at Re_lam_leave";
  ...
  SI.Pressure dp_lam_leave=A*C1*zeta_LOC*(B/2)*max(1, min(1.4, (lambda_lam_leave_rough/
      ...
      *v_lam_leave^(pow) "Pressure loss at end of transition to surface roughness regime";

  TYP.DarcyFrictionFactor lambda_turb_min_rough=0.25/(Modelica.Math.log10(k/(3.7*
      IN_con.d_hyd) + 5.74/Re_turb_min^0.9))^2 "Darcy friction factor considering surface roughness at starting transition to constant turbulent regime";
  TYP.DarcyFrictionFactor lambda_turb_min_smooth=0.25/(Modelica.Math.log10(5.74/Re_turb_min^0.9))^2 "Darcy friction factor neglecting surface roughness at starting transition to constant turbulent regimee";

  TYP.DarcyFrictionFactor lambda_turb_max_rough=0.25/(Modelica.Math.log10(k/(3.7*
      IN_con.d_hyd) + 5.74/Re_turb_max^0.9))^2 "Darcy friction factor considering surface roughness at starting transition to constant turbulent regime";
  TYP.DarcyFrictionFactor lambda_turb_max_smooth=0.25/(Modelica.Math.log10(5.74/Re_turb_max^0.9))^2 "Darcy friction factor neglecting surface roughness at starting transition to constant turbulent regimee";

  SI.Velocity v_turb_min = Re_turb_min*IN_var.eta/(IN_var.rho*d_hyd) "Mean velocity for starting of transition to constant turbulent regime";

  SI.Velocity v_turb_max = Re_turb_max*IN_var.eta/(IN_var.rho*d_hyd) "Mean velocity for end of transition to constant turbulent regime";
  ...
  SI.Pressure dp_turb_min=A*C1*zeta_LOC*(B/2)*max(1, min(1.4, (lambda_turb_min_rough/
      ...
      *v_turb_min^(pow) "Pressure loss at starting of transition to constant turbulent regime";

  SI.Pressure dp_turb_max=A*C1*zeta_LOC*max(1, min(1.4, (lambda_turb_max_rough/
      lambda_turb_max_smooth)))*IN_var.rho/2*v_turb_max^2 "Pressure loss at end of transition to constant turbulent regime";
  ...
  TYP.DarcyFrictionFactor lambda_FRI_smooth=0.25/(Modelica.Math.log10(5.74/max(Re_lam_min, Re_smooth)^0.9))^2 "Darcy friction factor neglecting surface roughness";

  //SOURCE_3: Lac 6, Figure 18
  Real CF_fri= SMOOTH(dp_lam_leave, dp_lam_min, abs(dp))*max(1, min(1.4, (lambda_FRI_rough/
      lambda_FRI_smooth))) + SMOOTH(dp_lam_min, dp_lam_leave, abs(dp)) "Correction factor for surface roughness";

  SI.Velocity velocity=v_smooth/max(1, CF_fri)^(0.5) "Corrected velocity considering surface roughness";
