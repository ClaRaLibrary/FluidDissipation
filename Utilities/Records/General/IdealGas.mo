...
record IdealGas "Base record for generic pressure loss function | ideal gas | mean density"
  ...
  parameter SI.SpecificHeatCapacity R_s(min=1) = 287 "Specific gas constant of ideal gas"
    annotation (Dialog(group="Fluid properties"));

  Real Km(min=Modelica.Constants.eps) = R_s*(2e3)/((10)^exp/rho_m) "Coefficient for pressure loss law [(Pa)^2/{(kg/s)^exp*K}]"
    annotation (Dialog(group="Generic variables"));
