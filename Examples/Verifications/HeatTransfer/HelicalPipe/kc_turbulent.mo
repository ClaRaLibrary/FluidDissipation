...
model kc_turbulent "Verification of function kc_turbulent"
  ...
  //input record
  FluidDissipation.HeatTransfer.HelicalPipe.kc_turbulent_IN_con m_flow_IN_con_1[n](
    each d_hyd=d_hyd,
    each L=L,
    each n_nt=n_nt[1],
    each h=h) annotation (Placement(transformation(extent={{-80,20},{-60,40}})));

  FluidDissipation.HeatTransfer.HelicalPipe.kc_turbulent_IN_var m_flow_IN_var_1[n](
    cp=cp,
    eta=eta,
    lambda=lambda,
    rho=rho,
  ...
  FluidDissipation.HeatTransfer.HelicalPipe.kc_turbulent_IN_con m_flow_IN_con_2[n](
    each d_hyd=d_hyd,
    each L=L,
    each n_nt=n_nt[2],
    each h=h) annotation (Placement(transformation(extent={{-20,20},{0,40}})));

  FluidDissipation.HeatTransfer.HelicalPipe.kc_turbulent_IN_var m_flow_IN_var_2[n](
    cp=cp,
    eta=eta,
    lambda=lambda,
    rho=rho,
  ...
  FluidDissipation.HeatTransfer.HelicalPipe.kc_turbulent_IN_con m_flow_IN_con_3[n](
    each d_hyd=d_hyd,
    each L=L,
    each n_nt=n_nt[3],
    each h=h) annotation (Placement(transformation(extent={{40,20},{60,40}})));

  FluidDissipation.HeatTransfer.HelicalPipe.kc_turbulent_IN_var m_flow_IN_var_3[n](
    cp=cp,
    eta=eta,
    lambda=lambda,
    rho=rho,
...
equation
  ...
  for i in 1:n loop
    (,,Re_1[i],Nu_1[i],) =
      FluidDissipation.HeatTransfer.HelicalPipe.kc_turbulent(m_flow_IN_con_1[i],
      m_flow_IN_var_1[i]);
  end for;

  for i in 1:n loop
    (,,,Nu_2[i],) = FluidDissipation.HeatTransfer.HelicalPipe.kc_turbulent(
      m_flow_IN_con_2[i], m_flow_IN_var_2[i]);
  end for;

  for i in 1:n loop
    (,,,Nu_3[i],) = FluidDissipation.HeatTransfer.HelicalPipe.kc_turbulent(
      m_flow_IN_con_3[i], m_flow_IN_var_3[i]);
  end for;

  annotation (__Dymola_Commands(file=
          "modelica://FluidDissipation/Extras/Scripts/heatTransfer/helicalPipe/kc_turbulent.mos" "Verification of kc_turbulent"),
